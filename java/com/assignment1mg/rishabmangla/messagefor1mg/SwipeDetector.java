...
/**
 * Created by r.mangla on 08-Jun-15.
 */
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(deltaX) > MIN_DISTANCE_X) {
                    // left or right
                    if (deltaX < 0) {
                        Log.i("rishab", "Swipe Left to Right");
                        mSwipeDetected = Action.LR;
                        return true;
                    }
                    if (deltaX > 0) {
                        Log.i("rishab", "Swipe Right to Left");
                    ...
                    if (Math.abs(deltaY) > MIN_DISTANCE_Y) {
                        // top or down
                        if (deltaY < 0) {
                            Log.i("rishab", "Swipe Top to Bottom");
                            mSwipeDetected = Action.TB;
                            return false;
                        }
                        if (deltaY > 0) {
                            Log.i("rishab", "Swipe Bottom to Top");
